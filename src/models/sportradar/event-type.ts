...
export enum EventType {
    ...
    // - Jump ball <reason>. <possessor> vs <challenger> (<possession> gains possession) jumpballviolation - <charged_to> jump ball violation
    JumpBall  = "jumpball",
    ...
    // An "open inbound" event in basketball refers to a situation where a team is inbounding the basketball
    // from out of bounds.
    // The term "open" might suggest that the player who is about to receive the inbound pass is not closely
    // guarded or marked by a defender, therefore they are "open"./
    ...
    // After a made basket: The ball is inbounded from anywhere along the baseline. The inbounder can move along
    // the baseline if the inbounding is after a made basket by the opponent.
    // After a dead-ball turnover or at the start of a quarter: The ball is inbounded from the sideline,
    // near the half-court line.
    // After a defensive rebound: The player who gets the rebound can directly put the ball into play without having
    // to inbound it.
    // After a timeout: Depending on the rules, the team can choose to inbound the ball from
    // the sideline or the baseline.
    // In all these situations, the objective is the same: to safely get the ball back in play and start
    // the team's offensive possession. A successful open inbound pass can often set up effective offensive plays.
    // - Open inbound <team_name>
    OpenInbound = "openinbound",

    // the method used to begin each half of a basketball game (and overtime periods, if necessary).
    // The referee will throw the ball straight up from the center of the court,
    // and one player from each team will try to tap it to their teammates, hence the term "tip-off".
